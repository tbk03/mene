...
MENE [Headline Report](https://assets.publishing.service.gov.uk/government/uploads/system/uploads/attachment_data/file/828552/Monitor_Engagement_Natural_Environment_2018_2019_v2.pdf): figures to try to recreate?

MENE [scrollytelly](https://defra.maps.arcgis.com/apps/Cascade/index.html?appid=d5fe6191e3fe400189a3756ab3a4057c): figures to try to recreate?

MENE Technical Report including [weighting](https://assets.publishing.service.gov.uk/government/uploads/system/uploads/attachment_data/file/875153/MENE_Technical_Report_Years_1_to_10v2.pdf) (page 27):

More [weighting info](http://publications.naturalengland.org.uk/publication/2248731?category=47018):

MENE [Dashboard](https://defra.maps.arcgis.com/apps/MapSeries/index.html?appid=2f24d6c942d44e81821c3ed2d4ab2ada):

**People and Nature**

[Weighting](https://assets.publishing.service.gov.uk/government/uploads/system/uploads/attachment_data/file/1021832/The_PeopleandNature_Survey_For_England_Weighting_Guidance.pdf)

[Infographic](https://assets.publishing.service.gov.uk/government/uploads/system/uploads/attachment_data/file/1020644/PANS_Year_1_summary_infographic.pdf) (2020/2021)

## Import data

```{r}

# home machine path: ../../../datasets/people_and_nature_survey/num_green_space_visits.xlsx
# work machine path: ../../datasets/people_and_nature_survey/num_green_space_visits.xlsx

visit_proportions <- readxl::read_xlsx("../../../datasets/people_and_nature_survey/num_green_space_visits.xlsx") %>% 
  mutate(date = lubridate::date(date),
         year = factor(lubridate::year(date)),
         month = lubridate::month(date),
         month_fct = factor(month, levels = 1:12, labels = month.abb),
         interim = factor(interim),
         est_perc_adults = round(est_perc_adults, 2))

ne_colour_pallette <- c("#126E96", "#0EAB4E")

visit_proportions %>% 
  filter(num_visits == "Any visits*" |
           num_visits ==  "Any visits**") %>% 
  
  ggplot() +
  
  geom_ribbon(aes(x = month_fct, ymin = ci_low , ymax = ci_high, 
                  fill = year, group = year), 
              alpha = 0.1) +
  
  geom_line(aes(x = month_fct, y = est_perc_adults,
                colour = year, group = year)) +
  
  geom_point(aes(x = month_fct, y = est_perc_adults,
                colour = year, group = year)) +
  
  geom_text(aes(x = month_fct, y = est_perc_adults,
                colour = year, group = year,
                label = round(est_perc_adults * 100, 0))) +
  #coord_flip() +
  #scale_x_continuous(breaks = 1:12) +
  scale_fill_manual(values =  ne_colour_pallette) +
  scale_colour_manual(values =  ne_colour_pallette) +
  coord_polar(start = 6) +
  #ylim(0, NA) +
  labs(x = NULL) +
